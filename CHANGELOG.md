...
## v1.7.24（2026-07-29，扫榜后两步选择：先选板块再选书 + 禁止跨题材混排）

- **闸 A「拆书」从一步改为两步**：
  - 第一步：展示 2-3 个推荐选题（题材标签+热度信号+为什么值得关注）→ AskUserQuestion 让用户选 1-2 个板块
  - 第二步：只展示所选板块下的候选书 → AskUserQuestion 让用户勾选 1-3 本带入拆书
- **禁止跨题材混排**：未选中板块的候选书不在第二步出现。新增硬规则：候选书必须与所属题材严格对应，禁止把其他题材的书塞进来凑数。
- `long-scan.md` Phase 4 新增「两步选择流程」说明段；闸 A 拆书分支重写为两步；`topic-decision.md` 模板「拆书候选书库」段加强分组约束说明。
