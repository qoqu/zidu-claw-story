...
function aggregate(projectDir) {
  ...
  return { name, chapters, totalWords, lastChapter, density, health, bank, updated: updatedStr };
}
...
function buildHtml(rows) {
  const cards = rows.map(r => {
    ...
    return `<div style="border:1px solid #eee;border-radius:10px;padding:14px 16px;margin:8px;width:280px;display:inline-block;vertical-align:top;box-shadow:0 1px 3px rgba(0,0,0,.05)">
  ...
  <div style="font-size:13px;color:#555;line-height:1.7">
    ...
    健康度：<b style="color:${hc}">${r.health.pct}%</b>（${r.health.ok}/${r.health.total}）<br>
    记忆沉淀：<b>${r.bank}</b> 条<br>
    更新：<span style="color:#888">${r.updated}</span>
  </div>
</div>`;
  }).join('');
