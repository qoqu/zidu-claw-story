...
当前版本：**1.7.22**（见 `VERSION`）。变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)。
...
- **题材库检索与扩充（T3）**：`genre-library.js` 检索 references/genres/ 下 37 篇按题材模板（list/search/filter/show/add/scaffold）；`genre-methodology.js` 按管线阶段 + 篇幅确定性召回 references/genre-* 跨题材写法方法论，开书/大纲阶段自动注入
...
- **自测套件**：selftest 给 52 脚本做语法/启动/功能三层冒烟，改一处不崩一片
...
> **SOP 图随 `SKILL.md` 主流程自动校验**：`SKILL.md` 的「主流程 SOP」段（带 `SOP-ANCHOR` 哨兵注释）是唯一权威源，指纹存于 `docs/sop-baseline.json`。`selftest.js` 每次自测会经 `scripts/audit.js --sop-check` 自动告警漂移；若主流程变更，重导出 `docs/sop-complete.svg`/`.html` 后运行 `node scripts/audit.js --update-baseline` 刷新基线即可，不再依赖人工记忆。
