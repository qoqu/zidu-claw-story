...
## 拆文字段 → 写作参考
读 `_meta.json.structure_counts` 时，按此表回查当前 skill 已登记的对应写作 reference。短篇优先走 genres/ 题材模板与 short-craft，长篇优先走长篇同类理论文件；未在当前 skill `参考资料` 表登记的文件不要跨 skill 加载。

| 拆文字段 | 含义 | 写作参考 |
|---------|------|---------|
| `beats` | 结构段（开端/发展/高潮/结局） | 当前 skill 的题材结构文件；短篇优先 genres/ 题材模板 / `genre-writing-formulas.md` |
| `hooks` | 钩子数 | `hooks-chapter.md` / `hooks-suspense.md`；短篇开头密度补 short-craft |
| `setup_clues` | 反转铺垫线索 | `reversal-toolkit.md` |
| `character_archetypes` | 反差人物 | 当前 skill 的人物/题材风格文件；短篇优先 genres/ 题材模板 / genre-writing-techniques |
