...
- **技能自带、无需部署**：`SKILL.md` + `references/`（扁平 .md）+ `scripts/`（48 个 .js）。主会话加载技能即全部可用。
- **可选 subagent（共 7 个）**：源文件在技能内 `references/setup_*.md`：
  - [`setup_story-architect.md`](setup_story-architect.md) → `story-architect`（题材/大纲/反转/情绪，opus）
  - [`setup_story-explorer.md`](setup_story-explorer.md) → `story-explorer`（只读结构化查询，haiku）
  - [`setup_story-researcher.md`](setup_story-researcher.md) → `story-researcher`（外部资料研究，sonnet）
  - [`setup_chapter-extractor.md`](setup_chapter-extractor.md) → `chapter-extractor`（章节摘要/情节点提取，haiku）
  - [`setup_character-designer.md`](setup_character-designer.md) → `character-designer`（角色设定/对话创作，sonnet）
  - [`setup_consistency-checker.md`](setup_consistency-checker.md) → `consistency-checker`（事实/伏笔一致性，只读，haiku）
  - [`setup_narrative-writer.md`](setup_narrative-writer.md) → `narrative-writer`（正文写作/去AI味，sonnet）
