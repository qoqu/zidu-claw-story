...
> 注：`quality-gate.js` 已内部调度 `check-ai-patterns`，所以写章流程里**不要在其外再单独跑 `check-ai-patterns.js`**，避免双重执行（见 SKILL.md「写章标准流程」步骤 3/4）。pacing 维度依赖 `追踪/追读力.md`，需每章 `tracking-updater reading-power` 供数，否则不触发；`reading-power` 带 `--qidian-rate/--fanqie-rate`（起点/番茄）或 `--real-rate` 时，pacing 信号改由平台真实追读率均值接管（结构性归一分回退为参考），任一平台真实率低于阈值即预警水章。
