...
### Phase 1：确认平台和方向（默认实时新书榜）

**默认行为：扫「实时新书榜」（各平台最新上架/新人向榜单）。**

用 AskUserQuestion 向用户确认两项信息：

1. **平台**：起点 / 番茄 / 晋江 / 盐言 / 其他（可多选）
2. **频道或题材范围**：男频 / 女频 / 特定题材标签 / 不限

> 默认值说明：
> - 若用户只说「扫榜」无更多限定，**默认 = 番茄 + 男频 + 实时新书榜**，直接开跑；跑完后展示结果再让用户决定是否加平台/换频道。
> - 若用户在启动闸已选了「📊 选」进入本流程，**不要重复问「你要扫榜吗」**——用户已选，直接进本 Phase 确认参数即可。
> - 各平台「实时新书榜」对应：起点=新人签约新书榜、番茄=新书榜、晋江=新入库/新人榜、盐言=最新收录。

关键判断：
- 用户已有明确题材方向 → 针对该方向 + 实时新书榜做定向扫榜
- 用户没有方向 → 全频道实时新书榜概览 + 找趋势
- 用户想跨平台比较 → 多平台实时新书榜对比分析
...
按 `topic-decision.md` 的选题四步产出 2-3 个推荐选题（能爆的原因 → 市场验证 → 差异化定位 → 可行性+失败风险+验证动作），写入**本次扫榜输出目录** `{outdir}/选题决策.md`，并告知用户路径。下一步**不是**自动开写——见下方「流程衔接」的**手动模式强制互动**：产出 `选题决策.md` 后必须停顿，用 AskUserQuestion 让用户先选「拆书 / 直接开书 / 再扫 / 短篇」。（若用户在同一句已明确说"开写 / 写第 N 章"，仍走「开书 → 咨询 → 写章」闸 B，不得跳过。）

**同步构建「拆书候选书库」（按题材分组）：** 对每个推荐选题，从本次实际扫榜条目中挑 2-4 本最贴合的对标书，每条含：书名 / 作者 / 平台·榜单 / 核心指标（在读数·月票·收藏等）/ 为什么适合当标杆（它集中体现了该选题的哪个爆点）。写入 `选题决策.md` 的「拆书候选书库」段，**按选题分组存放**（每个选题一个子段，书与题材严格一一对应）。

> **两步选择流程（闸 A「拆书」分支必须遵循）：**
>
> **第一步 — 选板块**：在对话中以列表展示 2-3 个推荐选题（每个含：题材标签、热度信号、为什么值得关注），用 AskUserQuestion 让用户勾选 1-2 个感兴趣的板块。
>
> **第二步 — 选书**：根据用户选中的板块，只展示**该板块下的候选书**（来自「拆书候选书库」的对应子段），用 AskUserQuestion 让用户勾选 1-3 本。再把所选书的「书名+平台」直接带入 `/story-long-analyze`。
>
> **严禁跨题材混排**：第一步未选中的板块，其候选书不在第二步出现。禁止把所有板块的书混在一起让用户选——这会导致用户在不了解的题材里乱选。
...
- **候选书与题材严格对应**：某选题的候选书必须是该题材方向的真书，禁止把其他题材的书塞进来凑数。
...
- **拆书（推荐先研究标杆，两步选择）：**
  - **第一步 — 选板块**：用 AskUserQuestion 列出 2-3 个推荐选题（题材标签 + 热度信号 + 为什么值得关注），让用户勾选 1-2 个感兴趣的板块。
  - **第二步 — 选书**：根据用户选中的板块，只展示该板块下的候选书（来自「拆书候选书库」对应子段），用 AskUserQuestion 让用户勾选 1-3 本。再把所选书的「书名+平台」直接带入 `/story-long-analyze`，坐实"能爆的原因"再设计自己的书（对齐 oh-story「先扫榜、后拆书」）。
  - **严禁跨题材混排**：未选中板块的候选书不在第二步出现。long-analyze 会接着向用户索取原文，不再重复问书名。
