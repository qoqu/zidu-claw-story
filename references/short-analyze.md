...
题材作为「对照标尺」加载——见 `references/short-analyze_genre-catalog.md` 等文件首段「## 用作
拆文标尺时」说明。
...
详细模板见 [output-templates.md](references/short-analyze_output-templates.md)，方法论见
[material-decomposition.md](references/short-analyze_material-decomposition.md)，输出契约见
...
扫描 `拆文报告.md` 全文 against [references/short-analyze_banned-words.md](references/short-analyze_banned-words.md)
词表 + [references/short-analyze_anti-ai-writing.md](references/short-analyze_anti-ai-writing.md) 句式规则。
...
[output-templates.md 质量检查必填字段](references/short-analyze_output-templates.md)。

质量标准的阈值、数值与计算方式的唯一权威定义见
[material-decomposition.md 质量标准](references/short-analyze_material-decomposition.md)。
...
| [references/short-analyze_output-templates.md](references/short-analyze_output-templates.md) | 拆文时：输出模板 + 结构库 + 质量检查（含 [BLOCK]/[WARN] 标注） |
| [references/short-analyze_material-decomposition.md](references/short-analyze_material-decomposition.md) | 拆文方法论：情节节点提取 + 写作手法 + 情感线 + 节奏分析 + 共鸣分析 + 人物规则 + **质量标准唯一权威** |
| [references/short-analyze_quality-checklist.md](references/short-analyze_quality-checklist.md) | 评估**源文**质量时：短篇拆书的质量自检清单（评估对象的好坏，不是评估拆文报告本身） |
| [references/short-analyze_anti-ai-writing.md](references/short-analyze_anti-ai-writing.md) | Phase 7.1：扫描**拆文报告本身**的 AI 腔（不是源文滤镜） |
| [references/short-analyze_banned-words.md](references/short-analyze_banned-words.md) | Phase 7.1：拆文报告禁用词速查 |
...
| [references/short-analyze_genre-catalog.md](references/short-analyze_genre-catalog.md) | 拆解特定题材时：加载对应题材的「短篇视角」章节作为标准模式 |
...
| [references/short-analyze_character-basics.md](references/short-analyze_character-basics.md) | 拆解人物基础设定时作为人设要素对照 |
| [references/short-analyze_character-design-methods.md](references/short-analyze_character-design-methods.md) | 拆解人物内在矛盾时作为三层标签反差对照（contradiction_axis 来源） |
| [references/short-analyze_character-relations.md](references/short-analyze_character-relations.md) | 拆解人物关系网时作为关系类型对照 |
| [references/short-analyze_genre-core-mechanics.md](references/short-analyze_genre-core-mechanics.md) | 拆解题材核心梗与循环机制时作为机制对照 |
| [references/short-analyze_genre-readers.md](references/short-analyze_genre-readers.md) | 拆解读者心理与期待管理时作为读者画像对照 |

### 补充资料（拆 Stage 6「可复用结构」时按需对照）

> **题材写作公式**：`references/short-analyze_genre-writing-formulas.md`（21 大题材公式作为
> 「这篇是否合标」的对照标尺）
> **通用写作技法**：`references/short-analyze_genre-writing-techniques.md`（情绪操控 / 感情线 /
