...
> **⚠️ WB / 无 agent 环境降级**：WorkBuddy 原生包不捆绑 story-* agent、无 `/story-*` 命令注册表；本文 `story-explorer` / `narrative-writer` / `story-researcher` / `consistency-checker` 等 agent 调用均为**主机增强项**，WB 下自动降级为主线程按本流程执行（Read/Grep 等价完成），不阻塞日更。
...
> 3. `设定/题材正文提示卡.md`（题材边界 / 核心逻辑 / 读者期待 / 节奏密度；缺失时用 `设定/题材定位.md` + `references/genre-prose-cards.md` 索引匹配并读取 `references/genres/{题材}.md` 单模板优先生成，`references/style-genre-modules.md` 通用流派兜底）
...
>
> **题材正文提示卡**：优先读 `设定/题材正文提示卡.md`；缺失时不阻塞，先从 `设定/题材定位.md` 精确匹配 `references/genre-prose-cards.md` 索引，并只读取 `references/genres/{题材}.md` 题材单模板（高/中/低置信照原卡标注），无命中再从 `references/style-genre-modules.md` 抽取通用流派短 `genre_prose_card`。题材模板只管题材味和正文取舍，不改细纲、不覆盖情绪/节奏权威文件，也不接管句长/标点等文风细节。题材模板只在写手心里校准取舍，模板名/题材标签/置信度/条目/合规自评一律不写进正文。
>
...
| 4 | `大纲/细纲_第{N}章.md` | 本章写作计划 | **必须先补建**（占位 stub 视为缺失），不允许跳过 |
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **2.3 对标模块/节奏/题材模板/文风召回**：
     ...
     - **题材正文提示卡召回**：主会话优先读 `设定/题材正文提示卡.md`；缺失则先读 `设定/题材定位.md` + `references/genre-prose-cards.md` 索引，按主题材精确匹配后只读取 `references/genres/{题材}.md` 单模板（如 都市脑洞 / 豪门总裁 / 年代 / 双男主；低置信模板必须在意图确认标注低置信，并要求同题材对标校准），无命中再读 `references/style-genre-modules.md` 通用流派模块。跨题材时主题材抽 3-5 条、辅题材抽 1-2 条，生成 `genre_prose_card`（题材边界、核心逻辑、读者期待、核心爽点/情绪、节奏密度、场景颗粒、禁止漂移、本章取舍、卡片置信度）。题材模板必须进入 narrative-writer prompt，但只传短摘要，并说明卡片只供内部题材校准、正文里不得出现卡片文字或合规自评
  ...
  - 每章写完后**立即更新**以下文件：
    ...
    - `追踪/追读力.md`：运行 `tracking-updater.js <项目> reading-power --chapter N --hook-type … --coolpoint …`（钩子/爽点/微兑现），为追读密度曲线与质量门禁 `pacing` 维度供数；**不填则这章追读维度永远为空**
...
7. **格式与退化防护（门禁之前先净化）**：先运行 `node scripts/punct-precheck.js 正文/第XXX章_*.md`（默认 `--quote-mode keep`）清理无功能省略号、破折号、双连字符和独立分隔线；再运行 `node scripts/punct-format.js 正文/第XXX章_*.md`（默认 `--fix`）做写后格式化——清 AI 特殊标点、不可见字符、按平台切换引号。**两者确定性与零依赖，由主线程执行，narrative-writer agent 不运行这些脚本。**
...
9. **风格护栏（advisory，不阻断，对应 canonical 写章 6 步第 5 步）**：如要防文风漂移，运行 `node scripts/drift-guard.js 正文/第XXX章_*.md --project <项目目录>`，仅提示、不阻断，可跳过。
...
当检测到细纲不存在、或文件为 `topic-to-book scaffold` 生成的占位 stub（含「状态：待补建」等占位标记、无实质章节蓝图）时，不能跳过，必须先补建实质内容再写正文。按以下步骤补建：
