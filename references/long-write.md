...
11. **禁用词扫描**：对照 `references/long-write_banned-words.md` 检查本章，一级词（高频AI腔）命中即替换；二级词（低频/语境相关）高频出现时替换，偶发可参考 `references/long-write_anti-ai-writing.md` 定性裁定
...
随后运行 `node scripts/punct-precheck.js 正文/第XXX章_*.md`（默认 `--quote-mode keep`）清理无功能省略号、破折号、双连字符和独立分隔线；盐言「」不受影响。narrative-writer agent 不运行这些脚本。
...
| 去AI味 | `references/long-write_anti-ai-writing.md` |
...
| 去AI味 | **`references/long-write_anti-ai-writing.md`**（AI指纹/核心规则/Show Don't Tell） | `references/long-write_banned-words.md`（禁用词扫描）· `references/long-write_quality-checklist.md`（成稿检查） |
