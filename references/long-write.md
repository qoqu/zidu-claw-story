...
**质量门禁（唯一硬门禁）**：本批正文写完后，主会话对每章运行 `node scripts/quality-gate.js 正文/第XXX章_*.md <项目目录> --genre <题材> --json`。exit 2/3（阻断 / 评分不足）先回正文改写并复跑；含 `pacing` 维度，最新章追读密度过低会 ⚠️ 预警。quality-gate 内部已含 `check-ai-patterns`（去 AI 味）与 `style-lint`，**无需在其外单独跑 `check-ai-patterns.js`**（避免双重执行）。
...
- 运行 `node scripts/check-degeneration.js --check 正文/第XXX章_*.md`：blocking（复读、截断、拒绝语、tier1 工程词泄漏）只重写受影响章节，最多 2 次；仍失败就报告证据让用户定夺。advisory 先看脚本例外；故事内系统/界面用语、弹幕刷屏、重复台词等有功能则保留。
