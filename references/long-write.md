...
>
> **⚠️ WB / 无 agent 环境降级（必读）**：WorkBuddy 原生包**不捆绑** story-* agent，也没有 `/story-*` 命令注册表；上述 `.claude/agents/` 等目录在 WB 下不存在，检查找不到时直接 solo/direct 执行（主线程按本文件流程走 Read/Grep），能力不缺失、不阻塞写作。本文所有 `story-*` agent 与 `/story-*` 命令均为**主机增强项**（Claude Code / OpenCode / Codex 等宿主提供时可加速，无则主线程等价完成）。完整适配规则见 SKILL.md「四、WB 适配」。
...
**开书默认停靠**：用户只说"开书/写大纲/帮我开书"时，完成 Phase 1→3 与首批 10 章细纲后停止，报告已生成文件和下一步命令；除非用户同一句明确说"并写第1章/写 N 章/日更"，否则不要自动进入 Phase 4 正文。（与 SKILL.md「手动模式人工闸」一致：题材/核心概念/书名/细纲草案需用户确认，不得 AI 自动建 stub 就写正文。）
...
> **题材方法论注入**：进入本阶段前可运行 `node scripts/genre-methodology.js route --stage character --len long` 确定性召回 `references/genre-*` 中「角色/核心梗」相关方法论并注入上下文（与 `references/genres/` 题材模板互补）。
...
角色设计延伸笔记（按需读取）：[`setup_character-basics.md`](setup_character-basics.md)（基础属性框架）、[`setup_character-design-methods.md`](setup_character-design-methods.md)（设计方法论）、[`setup_character-relations.md`](setup_character-relations.md)（人物关系网）。双男主（BL）长篇变体见 [`long-write_双男主.md`](long-write_双男主.md)。
- 姓名：{}
- 年龄：{}
- 核心特质：{2-3 个关键词}
- 金手指/核心能力：{}
...
- **设定/题材正文提示卡.md**：从 `设定/题材定位.md` + `references/genre-prose-cards.md`（索引）+ `references/genres/`（单题材正文模板目录，按题材分类优先）+ `references/style-genre-modules.md`（通用流派补充）抽取本书正文层题材模板，只写题材边界、核心逻辑、读者期待、核心爽点/情绪、正文落点、前中后期打法、节奏密度、场景颗粒、禁止漂移；不写通用格式规则，不覆盖 `设定/文风.md`
...
> **题材方法论注入**：进入本阶段前可运行 `node scripts/genre-methodology.js route --stage outline --len long` 确定性召回 `references/genre-*` 中「题材框架/核心梗/写作公式/趋势」相关方法论并注入上下文，辅助卷纲与细纲设计。
...
> **题材方法论注入**：动笔前可运行 `node scripts/genre-methodology.js route --stage writing --len long` 确定性召回 `references/genre-*` 中「写作技巧/文风卡片/写作公式/核心梗」相关方法论并注入上下文，作为本章写作参考。
...
| 设定/题材正文提示卡.md | 全书/题材 | Phase 2（缺失则 Phase 4 写前即时生成） | Phase 4 每章写作前：按 `genre-prose-cards.md` 索引匹配后读取 `genres/` 目录对应单题材模板优先、`style-genre-modules.md` 通用模块兜底，与通用正文要求、情绪/节奏召回和文风一起组装 prompt |
...
8. **`设定/题材正文提示卡.md` 缺失** → 不阻塞；写前从 `设定/题材定位.md` 精确匹配 `references/genre-prose-cards.md` 索引，并只读取 `references/genres/` 中对应题材单模板（高/中/低置信照原卡标注），无命中再用 `references/style-genre-modules.md` 通用流派模块即时生成短 `genre_prose_card`。只有 `设定/题材定位.md` 也缺失时，退回细纲和目标平台做低置信题材模板，并在意图确认写明。
...
1. **检查细纲**：读取 `大纲/细纲_第{N}章.md`。如果不存在、或仅为 `topic-to-book scaffold` 生成的占位 stub（含「状态：待补建」占位标记、无实质章节蓝图），**必须先补建细纲再写正文**，不允许跳过细纲直接写作。补建时参考卷纲中本章对应的事件规划和上下文，并按新版“章节蓝图”模板补齐阶段位置、结构公式、禁止提前释放、内容概括、情节安排、人物关系/出场顺序、情节细化、结尾设定；旧版细纲缺这些字段不阻塞读取，但本轮若要回填，未知项写 `[待补充]`。
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果项目已部署 story-explorer agent（优先检查 `.claude/agents/story-explorer.md` 是否存在；不存在时再检查 `.opencode/agents/`，再不存在时检查 `.codex/agents/`），可 spawn `Agent(subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章")` 一次获取上下文）：
   ...
   - (14) `设定/题材正文提示卡.md`（如存在）— 本书正文层题材模板；缺失时从 `设定/题材定位.md` + `references/genre-prose-cards.md` 索引 + `references/genres/` 单题材模板目录（按题材分类优先）+ `references/style-genre-modules.md`（兜底）即时生成 `genre_prose_card`，不阻塞写作
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - 3.2 **模块召回、题材模板与文风召回**：
     ...
     - (c) **题材正文提示卡召回**：优先读 `设定/题材正文提示卡.md`；缺失则先读 `设定/题材定位.md` + `references/genre-prose-cards.md` 索引，按主题材精确匹配后只读取 `references/genres/` 中对应单题材模板（如 都市脑洞 / 豪门总裁 / 年代 / 双男主；低置信模板必须在意图确认标注低置信，并要求同题材对标校准），无命中再读 `references/style-genre-modules.md` 通用流派模块。跨题材时主题材抽 3-5 条、辅题材抽 1-2 条，生成短 `genre_prose_card`（题材边界、核心逻辑、读者期待、核心爽点/情绪、正文落点、前中后期打法、节奏密度、场景颗粒、禁止漂移、本章取舍、卡片置信度）。题材模板只约束正文层题材味，不改细纲剧情、不覆盖 `selected_emotion_module` / `rhythm_reference` / `设定/文风.md`；只在内部校准取舍，正文里不得出现模板名/标签/置信度/条目/合规自评
...
11. **禁用词扫描**：对照 `references/long-write_banned-words.md` 检查本章，一级词（高频AI腔）命中即替换；二级词（低频/语境相关）高频出现时替换，偶发可参考 `references/anti-ai-writing.md` 定性裁定
12. **更新追踪**：写完后即时更新 `追踪/伏笔.md`（新增/回收伏笔）、`追踪/时间线.md`（记录事件时序）和 `追踪/角色状态.md`（如本章引起角色状态变化——身份、能力、关系、公众形象——则更新对应角色条目并追加变更记录）。**同时运行 `tracking-updater.js <项目> reading-power --chapter N --hook-type … --coolpoint …`（钩子/爽点/微兑现）写入 `追踪/追读力.md`**，为追读密度曲线与质量门禁的 `pacing` 维度供数——不填则这章追读维度永远为空，review 与 pacing 预警都不会触发。本章若首次引入会复用的具名角色/势力，按 Phase 3「细纲后设定补全」规则补建对应 `设定/` 档案。角色状态更新规则详见 state-tracking.md。
...
检查两个维度：(1) **情绪交付**——每章是否交付了细纲中规划的目标情绪？(2) **技术质量**——一致性、格式、禁用词。参考 [references/long-write_quality-checklist.md](references/long-write_quality-checklist.md) 中的通用检查和长篇专项清单。

通用质量清单另见 [`setup_quality-checklist.md`](setup_quality-checklist.md)。
...
**格式与退化防护（门禁之前先净化）**：
- 运行 `node scripts/punct-precheck.js 正文/第XXX章_*.md`（默认 `--quote-mode keep`）清理无功能省略号、破折号、双连字符和独立分隔线；盐言「」不受影响。narrative-writer agent 不运行这些脚本。
- 运行 `node scripts/check-degeneration.js --check 正文/第XXX章_*.md`：blocking（复读、截断、拒绝语、tier1 工程词泄漏）只重写受影响章节，最多 2 次；仍失败就报告证据让用户定夺。advisory 先看脚本例外；故事内系统/界面用语、弹幕刷屏、重复台词等有功能则保留。净化后再跑下方门禁，确保判定基于干净文本。

**质量门禁（唯一硬门禁）**：格式净化后立即对每章运行 `node scripts/quality-gate.js 正文/第XXX章_*.md <项目目录> --genre <题材> --json`。exit 2/3（阻断 / 评分不足）先回正文改写并复跑（退化防护已前置，门禁为终检）；含 `pacing` 维度，最新章追读密度过低会 ⚠️ 预警。quality-gate 内部已含 `check-ai-patterns`（去 AI 味）与 `style-lint`，**无需在其外单独跑 `check-ai-patterns.js`**（避免双重执行）。

**风格护栏（advisory，不阻断）**：如需防文风漂移，运行 `node scripts/drift-guard.js 正文/第XXX章_*.md --project <项目目录>`，仅提示、不阻断。
...
**记忆沉淀与自动备份（写章收尾第 6 步）**：
- 沉淀好写法：`node scripts/learn-bank.js <项目目录> add --type 爽点套路 --content "..." [--chapter N]`（LLM 从本章抽好用写法写入 `记忆/写法沉淀.json`，新章任务书 `query` 召回，越写越香）。
- 自动备份：`node scripts/pipeline-gate.js backup <项目目录> --chapter N`（每章写完后轮转快照，保留最近 10 份，误删可恢复）。
...
| 场景 | 加载文件 |
|------|---------|
| 确定题材类型 | `references/long-write_genre-catalog.md` |
| 判断市场方向 | `references/long-write_genre-readers.md` |
...
| 场景 | 加载文件 |
|------|---------|
| 设定人物 | `references/long-write_character-basics.md` |
| 设计关系 | `references/long-write_character-relations.md` |
| 题材框架与定位 | `references/long-write_genre-catalog.md` + `references/long-write_genre-core-mechanics.md` |
...
| 题材写作公式 | `references/long-write_genre-writing-formulas.md` |
...
| 题材正文提示卡 / 题材分类卡 | `references/genre-prose-cards.md` 索引 + `references/genres/` 单题材模板目录（按题材分类优先） + `references/style-genre-modules.md`（通用流派补充） |
...
| 人物深化 | `references/long-write_character-design-methods.md` |
...
| 场景 | 加载文件 |
|------|---------|
| 质量检查 | `references/long-write_quality-checklist.md` |
| 禁用词扫描 | `references/long-write_banned-words.md` |
...
|------|-----------------|----------------------|
| 爽点（按意图分流） | **`references/plot-emotion-system.md`**（爽点设计体系：本质/六种类型/倒推法——"怎么设计爽点"先读这个） | 翻盘/高潮式爽点→`references/plot-core-methods.md`（假胜→崩解）· 打脸/装逼释放→`references/style-combat-face.md`· 题材打脸逆袭公式→`references/long-write_genre-writing-formulas.md`· 爽文循环/多层→`references/outline-methods.md`·`references/outline-conflict.md` |
...
| 感情线 | **`references/long-write_character-relations.md`**（好感度体系/四阶段 + 男女频差异） | `references/outline-conflict.md`（感情线设计）· `references/style-combat-face.md`（后宫文女主 / 男频极简爱情线构型）· `references/plot-special-topics.md`（爱情线提纯策略） |
| 反转 | **`references/reversal-toolkit.md`**（反转类型/铺垫/有效性自检） | `references/plot-core-methods.md`（假胜：先给希望再击碎） |
| 人物 | **`references/long-write_character-basics.md`**（主角/配角/反派/动机模板速填） | `references/long-write_character-design-methods.md`（三层标签反差/九维深化）· `references/long-write_character-relations.md`（关系类型/感情线） |
| 女频写作 | **`references/female-audience-writing.md`**（女频长篇：核心原则/文案/题材/感情线长线/平台） | `references/long-write_genre-readers.md`（读者心理/平台差异）· `references/long-write_character-relations.md`（感情线总框架） |
| 去AI味 | **`references/anti-ai-writing.md`**（AI指纹/核心规则/Show Don't Tell） | `references/long-write_banned-words.md`（禁用词扫描）· `references/long-write_quality-checklist.md`（成稿检查） |
