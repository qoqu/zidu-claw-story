...
本 skill 是一套「全流程一条龙」工具，但**手动模式（用户在线对话）下，创意分支必须由用户拍板，AI 不得替用户做决定**（用户工作模式规则：选热点 / 定角度 / 定平台 / 定设计 均属分支决策）。落实为一条硬规则：
...
- **自动模式（定时 / 用户不在场）可跳过此闸**：AI 自行决策所有分支，按目标平台适配格式。

> 误区纠正：实测中 AI 在用户只说了「长篇·从零开书·番茄」后，自行锁定题材、设计概念、写第 1 章并陷入去味循环——违反本规则。正确做法：开书停靠在「骨架 + 草案」，等用户签字再写。
