...
当本 skill 被加载，**无论用户用什么词启动**——「用一下 zidu-claw-story」「帮我写小说」「帮我扫个榜」「写一本都市文」等任何表述——你**必须**：
...
> **核心原则：label 本身就是用户看到的全部文字。** 功能说明必须直接写在 label 里，不能依赖分离的 description 字段（该字段在部分 UI 中不渲染）。
> **必须列出全部 8 个域，严禁合并/省略/折叠到「其他」。**

| label（AskUserQuestion 的选项标签，原样填入） |
|---|
| ✍ 写 — 开书写文：长篇/短篇开书 → 大纲 → 连载/日更/续写 → 完结 |
| 🔍 拆 — 拆书研究：拆解对标爆款结构、黄金三章分析、情绪弧研究 |
| 📊 选 — 扫榜选题：爬取起点/番茄/晋江/盐言等平台榜单，找热门题材和蓝海 |
| ✨ 净 — 净化美化：去 AI 味润色、生成小说封面图 |
| 🟥 查 — 查体导入：项目审查体检、导入已有小说、初始化写作环境 |
| 🛡 控 — 追踪管控：质检门禁、伏笔/角色/时间线追踪、追读力监控、备份续跑 |
| 🔁 流 — 流程管线：选题→成书闭环、排行榜蓝海指数、风格护栏、自测回归 |
| 📦 扩 — 扩展工具：题材库(37模板)检索、生成本书设定卡、多平台发布物料包 |

> 实施要点：
> - **必须用 AskUserQuestion（或等效的显式选项停等）**，不能只输出文字列表等用户自己说。
> - **label 必须包含功能说明文字**（格式：`emoji 单字 — 一句话说明`），不能只写 emoji+单字。用户靠 label 理解每个域是干嘛的。
> - **8 个域必须全量列出，一个不少**。禁止因为「选项太多」而只列 4 个、把其余折入「其他」或「其他补充…」——这是违规行为，用户需要看到完整地图才能做决策。
> - 用户选完后，再展开对应域的功能总览表（下方「功能总览」作为**按需展开的参考**，不是启动时全量展示）。
> - **无条件触发**：不按启动词的「明确程度」判断——即便用户已点名功能（如「帮我扫个榜」），也先走本闸确认本轮功能，再进对应流程。
...
本 skill 是一条龙工具，但**手动模式（用户在线对话）下，创意分支必须由用户拍板，AI 不得替用户做决定**（用户工作模式规则：选热点 / 定角度 / 定平台 / 定设计 均属分支决策）。落实为两道硬规则——**闸 A 在上游、闸 B 在下游**，两道都要求「强制互动」（用 AskUserQuestion 或显式选项停等用户表态）：

**闸 A · 扫榜后选择（拆书 / 直接开书）—— 强制互动**
- 扫榜（scan + 蓝海分析）跑完、产出 `选题决策.md` 后，**必须暂停**，用 AskUserQuestion（或显式选项）向用户抛出选择：
  - **A1 拆书**：选 1-3 本对标书深度拆解（`long-analyze`），把爆款结构 / 钩子 / 情绪弧研究透，再决定怎么写（对齐 oh-story「先扫榜、后拆书」的理念）；
  - **A2 直接开书**：基于蓝海信号直接 scaffold 开书骨架（`topic-to-book`），随后进入闸 B。
- **未经用户明确选 A1 或 A2，禁止自动进入拆书管道或写正文**。AI 可在选项里给推荐项（如「基于蓝海指数，推荐先拆 2 本对标再开书」），但**不得替用户拍板**。
- 这道闸在「开书 → 咨询 → 写章」闸之前；无论选哪条，下游仍有各自的人工闸。

**闸 B · 开书 → 咨询 → 写章**（沿用）
- **开书 = 搭骨架 + 抛选题建议**，不是「自动写完一本书」。scaffold / 选题→成书 只产出：项目骨架 + 拟定的 题材 / 核心概念 / 书名 / 细纲草案，并附上扫榜 / 蓝海依据。
- **咨询 = 暂停等签字**：把上述草案摆出来，让用户确认或提出修改；**未经用户明确「开写 / 写第 N 章」之前，禁止写任何正文**。
- **细纲须用户确认**：每章细纲是写正文的硬前置（无纲禁写），且 manual 下细纲草案需用户过目认可，不得 AI 自动建 stub 后就直接写正文。
- **自动模式（定时 / 用户不在场）可跳过两道闸**：AI 自行决策所有分支，按目标平台适配格式。

> 误区纠正：实测中 AI 在用户只说了「长篇·从零开书·番茄」后，直接扫榜、自行锁题材、设计概念、写第 1 章并陷入去味循环——违反本规则（既没在扫榜后停顿选路，也没在开书后等签字）。正确做法：扫榜后先停（闸 A 让用户选拆书 / 开书），开书只停靠「骨架 + 草案」等签字（闸 B）。
...
```
- 写完一章跑一次，复用 `style-drift.js --json` 但**只聚焦传入章节**的 z-score：从结果里定位该章号对应的指标，标记漂移（默认 advisory，不阻断）。可选 `--strict`：漂移时 exit 2 阻断，作 CI 预提交钩子。
