...
node scripts/tracking-updater.js <项目目录> reading-power --chapter N --hook-type 危机钩 --hook-strength strong --coolpoint "..." [--coolpoint "..."] --micropayoff "..." [--hard-violation HARD-xxx] [--debt 0] [--real-rate 12.3] [--real-finish 38.5]
```
...
- **有效密度 eff = 填了「真实追读率」则用真实率（已是 0-100）接管 pacing 信号，否则回退结构性归一分**。`--real-rate`/`--real-finish` 来自平台作家后台手抄（方案①），不填则纯结构性代理，完全兼容。
- 终端输出 ASCII 曲线 + **水章标记**（有效密度 < 阈值，默认 45）；`--html` 出 SVG 折线图（蓝=结构性归一分、橙虚线=真实率、红=水章）。
...
4. **写章标准流程（WB 手动版，唯一 canonical 流程）**：开书时从 `references/genres/` 选题材模板作为设定基底，之后**每章严格按以下 6 步**——与 `references/long-write.md` Phase 5、`references/workflow-daily.md` Step 3 完全一致，不要各写一套：
   1. 写正文（Phase 4）
   2. `tracking-updater.js after-chapter` + **`reading-power`**（喂追读密度数据；不填则 review / 门禁的追读维度永远为空；可加 `--real-rate N` 从平台后台手抄真实追读率，让真实数据接管 pacing 信号）
