...
> **重要：以下 `label` 和 `description` 必须原样填入 AskUserQuestion 的对应字段**——label 用短名（emoji+单字），description 用下方完整说明，**不要省略 description 或只填单字**。

| label | description（必须原样展示给用户） |
|---|---|
...
> - **每个选项必须同时填写 `label`（短名）和 `description`（功能说明）**——description 不能省略、不能只填单字，用户需要靠它理解每个域是干嘛的。
